...
#         print("output_feature : ", output_feature.shape)
        ...
        cur_idx = 0
        entity_graph_feature = None
        for i in range(len(graphs)): # batch 단위로 돌아감
            # average mention -> entity
            select_metrix = entity2mention_table[i].float()  # [local_entity_num, mention_num]
            select_metrix[0][0] = 1
            mention_nums = torch.sum(select_metrix, dim=-1).unsqueeze(-1).expand(-1, select_metrix.size(1))
...
#         print("entity_graphs: ", entity_graphs)
        path_info = self.dropout(
            self.activation(
                self.path_info_mapping(path_info)
            )
        )
#         print("h_entity: ", h_entity.shape)
#         print("t_entity: ", t_entity.shape)
...
#         print("output_feature",output_feature.shape)
#         print("entity_graph_feature",entity_graph_feature.shape)
#         print("h_entity",h_entity.shape)
#         print("t_entity",t_entity.shape)
